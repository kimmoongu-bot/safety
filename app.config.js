...
function resolve(config, env = {}) {
  ...
  return {
    ...
    name: `${config.name} (개발)`,
    /**
     * 앱이 스스로 "나는 개발용" 이라고 알 수 있게 한다.
     *
     * 개발용에만 보이는 것이 있다 — 지금은 채우기 화면의 '칸 정보'
     * (docs/자동완성.md 19장). 배포판에 남의 앱 화면 구조를 보여 주는 자리가
     * 있으면 안 된다.
     *
     * 꾸러미 이름의 `.dev` 를 보고 판단하지 않는다. 그건 안드로이드에만 있는
     * 것이고, 무엇보다 **이 파일이 정하는 것을 저쪽에서 되짚어 읽는 꼴**이라
     * 한쪽만 바뀌면 조용히 어긋난다.
     */
    extra: { ...config.extra, devBuild: true },
    android: {
      ...config.android,
      /**
       * 꾸러미 이름을 달리한다.
