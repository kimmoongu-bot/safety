...
export type VaultPayload = {
  ...
  prevPassword?: string;
  /**
   * 이 항목을 쓰는 곳 (docs/자동완성.md 24장).
   *
   * 자동 완성에서 사용자가 한 번 고르면 그 자리를 여기 적어 둔다. 다음부터는
   * 같은 곳에서 이 항목이 맨 위에 온다. `실손24` 라고 적어 둔 항목이
   * `silson24.or.kr` 과 만나지 못하던 문제를 이렇게 푼다 — 글자를 견주는 대신
   * **사용자가 한 번 알려 준 것**을 기억한다.
   *
   * 적는 모양은 `autofillMatch.siteKey` 가 정한다 (`web:...` / `app:...`).
   *
   * **없어도 된다.** 예전에 담아 둔 항목에는 이 자리가 아예 없고, 그대로 잘
   * 열린다. 구조 번호를 올리지 않은 이유다 — 있으면 쓰고 없으면 만다.
   */
  sites?: string[];
};
